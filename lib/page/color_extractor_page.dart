...
class _ColorExtractorPageState extends State<ColorExtractorPage> {
  ...
  Future<void> pickImageAndExtract() async {
    ...
    try {
      ...
      final List<Color> mainColors = ImageColorExtractor.extractMainColors(bytes);
      if (mainColors.isEmpty) {
        _showSnackBar('未提取到有效主色调');
        return;
      }
      final List<List<Color>> schemes = ImageColorExtractor.generateColorSchemes(mainColors.first);
  ...
  Future<void> extractFromAssetImage() async {
    try {
      final ByteData data = await rootBundle.load('assets/test.jpg'); // 替换为你的图片路径
        ...
        child: Column(
          ...
          children: [
            ...
            if (_mainColors.isNotEmpty) ...[
              ...
              const Text('提取的主色调', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                child: Row(
                  children: _mainColors.map(ColorDisplayUtil.buildColorBlock).toList(),
                ),
              ),
            ],
            ...
            if (_colorSchemes.isNotEmpty) ...[
              const SizedBox(height: 30),
              const Text('生成的配色方案', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              ..._colorSchemes.asMap().entries.map((entry) {
                ...
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(schemeNames[entry.key], style: const TextStyle(fontWeight: FontWeight.w500)),
                    const SizedBox(height: 5),
                    SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: Row(
                        children: entry.value
                            .map((color) => ColorDisplayUtil.buildColorBlock(color, size: 80))
                            .toList(),
                      ),
                    ),
                    const SizedBox(height: 20),
              ...
              // 配色效果示例
              const SizedBox(height: 30),
              const Text('配色效果示例', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              Container(
                width: double.infinity,
                height: 100,
                ...
                child: const Center(
                  child: Text('渐变效果', style: TextStyle(color: Colors.white, fontSize: 20)),
                ),
              ),
            ],
          ],
